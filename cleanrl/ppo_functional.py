...
def parse_args():
    ...
    # PPO specific arguments for Learning Rate
    ...
    ### Deal with the Steps and batch size 
...
def ppo_functional(
    ...
    logging.info(f"PyTorch version is {torch.__version__}")
    logging.info(f"PyTorch build is {torch.__config__.show()}")
    logging.info(f"PyTorch cuda is {torch.version.cuda}")
    logging.info(f"PyTorch cudnn is {torch.backends.cudnn.version()}")
    logging.info(f"PyTorch is cuda available {torch.cuda.is_available()}")
    logging.info(f"PyTorch is cuda enabled {torch.cuda.is_enabled()}")
