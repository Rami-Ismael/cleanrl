...
import gym
import numpy as np
from LTH_utils import compute_stats, copy_weights_linear, copy_weights_mlp, prune_mlp
...
def parse_args():
    ...
    parser.add_argument("--wandb-project-name", type=str, default="test-project",
...
def td3_functional(
    ...
    ## Create an copy of the NN
    actor_copy = Actor(envs).to(device)
    qf1_copy = QNetwork(envs).to(device)
    qf2_copy = QNetwork(envs).to(device)
    qf1_target_copy = QNetwork(envs).to(device)
    qf2_target_copy = QNetwork(envs).to(device)
    target_actor_copy = Actor(envs).to(device)
    actor_copy.load_state_dict(actor.state_dict())
    qf1_copy.load_state_dict(qf1.state_dict())
    qf2_copy.load_state_dict(qf2.state_dict())
    qf1_target_copy.load_state_dict(qf1.state_dict())
    qf2_target_copy.load_state_dict(qf2.state_dict())
    target_actor_copy.load_state_dict(actor.state_dict())
    ...
    obs = envs.reset()        
    assert args.prune_ratio >= 0 and args.prune_ratio <= 1, "prune_ratio should be in [0, 1]"
    per_round_ratio =  1 - (1 - args.prune_ratio) ** (1 / args.prune_iter)
    per_round_max_iter = int(args.total_timesteps / args.prune_iter)
    for global_step in range(args.total_timesteps):
        ...
        if global_step > 0 and global_step % per_round_max_iter == 0 :
            ## prune the mlp
            prune_mlp( actor , [per_round_ratio] * len(actor.model) , method = args.prune_method )
            prune_mlp( qf1 , [per_round_ratio] * len(qf1.model) , method = args.prune_method )
            prune_mlp( qf2 , [per_round_ratio] * len(qf2.model) , method = args.prune_method )
            prune_mlp( target_actor , [per_round_ratio] * len(target_actor.model) , method = args.prune_method )
            prune_mlp( qf1_target , [per_round_ratio] * len(qf1_target.model) , method = args.prune_method )
            prune_mlp( qf2_target , [per_round_ratio] * len(qf2_target.model) , method = args.prune_method )
            
            copy_weights_mlp( actor_copy , actor )
            copy_weights_mlp( qf1_copy , qf1 )
            copy_weights_mlp( qf2_copy , qf2 )
            copy_weights_mlp( target_actor_copy , target_actor )
            copy_weights_mlp( qf1_target_copy , qf1_target )
            copy_weights_mlp( qf2_target_copy , qf2_target )
            stats = compute_stats(actor) 
            for name , stat in stats.items():
               writer.add_scalar( "stats/actor/" + name , stat , global_step )
